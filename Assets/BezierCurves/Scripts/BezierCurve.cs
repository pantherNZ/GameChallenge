...
public class BezierCurve : MonoBehaviour {
    ...
    {
        return GetPointAt( t, out var _ );
    }

    public Vector3 GetPointAt( float t, out Vector3 direction )
	{
        if( t <= 0 )
        {
            direction = GetPointAt( 0.01f ) - points[0].position;
            return points[0].position;
        }
        else if( t >= 1 )
        {
            direction = points[points.Length - 1].position - GetPointAt( 0.99f );
		...
		for(int i = 0; i < points.Length - 1; i++)
		{
			curvePercent = ApproximateLength(points[i], points[i + 1], 32) / length;
        ...
        var pos = GetPoint( p1, p2, t / curvePercent );
        direction = pos - GetPoint( p1, p2, t / curvePercent - 0.01f );
        return pos;
